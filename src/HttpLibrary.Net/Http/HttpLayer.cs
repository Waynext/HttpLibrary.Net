...
using HttpLibrary.Requesting.Help;
...
namespace HttpLibrary.Http
{
    ...
    class HttpLayer
    {
        ...
        private IHttpLibraryPlatform httpLibPlatform;
        public HttpLayer(IHttpLibraryPlatform httpLibPlatform)
        {
            this.httpLibPlatform = httpLibPlatform;

            requestTimerDict = new Dictionary<HttpRequest, DateTime>();

            httpLibPlatform.Timer.StartTimer(TimeSpan.FromMilliseconds(10 * 1000), RequestTimer);
            ...
            lock (timerLocker)
            {
                ...
                foreach (var req in requests)
                {
                    ...
                    if (reqTime.TotalMilliseconds > sendRequestTimeoutInMillseconds)
                    {
                        ...
                        Debug.WriteLine("Http Request timeout " + LogRequetId(req.RequestId));
                    }
                }
            }
        ...
        public void SendRequest(HttpRequest request)
        {
            ...
            if (request.RequestStreams != null)
            {
                try
                {
                    //LogHelper.OnlineLogger.InfoFormat("Send Http Request Stream to {0}. {1}", originalReq.RequestUri.ToString(), LogRequetId(request.RequestId));
                    Debug.WriteLine("Send Http Request Stream to {0}. {1}", originalReq.RequestUri.ToString(), LogRequetId(request.RequestId));
                    var result = originalReq.BeginGetRequestStream(new AsyncCallback(OnRequestStreamReady), request);
                    //LogHelper.OnlineLogger.InfoFormat("Sent {0}. {1}", originalReq.RequestUri.ToString(), LogRequetId(request.RequestId));
                    Debug.WriteLine("Sent {0}. {1}", originalReq.RequestUri.ToString(), LogRequetId(request.RequestId));
                    AddRequestTimer(request);
                    
                }
                catch (Exception ex)
                {
                    //LogHelper.OnlineLogger.Error("Can not send http request " + LogRequetId(request.RequestId), ex);
                    Debug.WriteLine("Can not send http request " + LogRequetId(request.RequestId));
            ...
            else
            {
                try
                {
                    //LogHelper.OnlineLogger.InfoFormat("Send Http Request to {0}. {1}", originalReq.RequestUri.ToString(), LogRequetId(request.RequestId));
                    Debug.WriteLine("Send Http Request to {0}. {1}", originalReq.RequestUri.ToString(), LogRequetId(request.RequestId));
                ...
                {
                    //LogHelper.OnlineLogger.Error("Can not begin get http response. " + LogRequetId(request.RequestId), ex);
                    Debug.WriteLine("Can not begin get http response. " + LogRequetId(request.RequestId));
                    Debug.WriteLine(ex.ToString());
                    request.Exception = ex;
                    ReportFailed(request);
                }
        ...
        private string GetProxyAuthenticate(HttpWebResponse resp)
        {
            ...
            var auth = resp.Headers[HttpRequestHeader.ProxyAuthorization];
            if (auth != null)
            {
            ...
            {
                var auths = resp.Headers[httpWWWAuthenticateKey];
                if (auths != null)
                {

                    var isBasics = auths.StartsWith("basic", StringComparison.OrdinalIgnoreCase);
                    if (auth != null && isBasics)
                    {
                        var basicParts = auths.Split(authSplitor);
        ...
        private void OnRequestStreamReady(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                finally
                {
                    ...
                    {
                        //LogHelper.OnlineLogger.Error("Close http request stream failed. " + LogRequetId(req.RequestId), ex);
                        Debug.WriteLine("Close http request stream failed. " + LogRequetId(req.RequestId));
                        Debug.WriteLine(ex.ToString());
                        try
                        {
                            req.OriginalRequest.Abort();
                        ...
                        {
                            //LogHelper.OnlineLogger.Error("Abort http request stream failed. " + LogRequetId(req.RequestId), ex2);

                            Debug.WriteLine("Abort http request stream failed. " + LogRequetId(req.RequestId));
            ...
            {
                //LogHelper.OnlineLogger.Error("Can not get http response. " + LogRequetId(req.RequestId), ex);
                Debug.WriteLine("Can not get http response. " + LogRequetId(req.RequestId));
                Debug.WriteLine(ex.ToString());
                req.Exception = ex;
                ReportFailed(req);
            }
        ...
        private bool SendRequestStreamInBlocks(Stream target, HttpRequest req)
        {
            ...
            try
            {
                ...
                var range = req.OriginalRequest.Headers[HttpRequestHeader.ContentRange];
                ...
                //LogHelper.OnlineLogger.DebugFormat("Start to send request stream, id={0}. {1}", targetHash, LogRequetId(req.RequestId));
                Debug.WriteLine("Start to send request stream, id={0}. {1}", targetHash, LogRequetId(req.RequestId));
                long fileLen = -1;
                try
                {
                ...
                {
                    //LogHelper.OnlineLogger.WarnFormat("Can not get file length, stream is disposed {0}. {1}", ex.ToString(), LogRequetId(req.RequestId));
                    Debug.WriteLine("Can not get file length, stream is disposed {0}. {1}", ex.ToString(), LogRequetId(req.RequestId));
                }
                ...
                if (fileLen != -1)
                {
                    ...
                    while (!req.IsStopped && reqBufEnumerator.MoveNext())
                    {
                        ...
                        {
                            //LogHelper.OnlineLogger.ErrorFormat("Send stream is failed in the middle, id = {0}, ex = {1}. {2}", targetHash, ex.ToString(), LogRequetId(req.RequestId));
                            Debug.WriteLine("Send stream is failed in the middle, id = {0}, ex = {1}. {2}", targetHash, ex.ToString(), LogRequetId(req.RequestId));
                        ...
                        {
                            //LogHelper.OnlineLogger.WarnFormat("Can not write stream, {0}. {1}", ex.ToString(), LogRequetId(req.RequestId));
                            Debug.WriteLine("Can not write stream, {0}. {1}", ex.ToString(), LogRequetId(req.RequestId));
                            throw new WebException(ex.Message, ex);
                        }

                        req.ReportProgress(reqBufEnumerator.Offset, fileLen, reqBufEnumerator.CurrentLength);
                        //LogHelper.OnlineLogger.DebugFormat("Sending at {0}/{1}, id={2}. {3}", reqBufEnumerator.Offset, fileLen, targetHash, LogRequetId(req.RequestId));
                        Debug.WriteLine("Sending at {0}/{1}, id={2}. {3}", reqBufEnumerator.Offset, fileLen, targetHash, LogRequetId(req.RequestId));
                    }
        ...
        private void OnResponseReady(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                {
                    //LogHelper.OnlineLogger.WarnFormat("Recevie respose exception {0}\n{1}. {2}", ex.ToString(), ex.Status, LogRequetId(req.RequestId));
                    Debug.WriteLine("Recevie respose exception {0}\n{1}. {2}", ex.ToString(), ex.Status, LogRequetId(req.RequestId));
                    if (ex.Response != null)
                    {
                ...
                //LogHelper.OnlineLogger.InfoFormat("Receive http response from {0}. {1}", httpResponse.ResponseUri.ToString(), LogRequetId(req.RequestId));
                Debug.WriteLine("Receive http response from {0}. {1}", httpResponse.ResponseUri.ToString(), LogRequetId(req.RequestId));
            ...
            {
                //LogHelper.OnlineLogger.ErrorFormat("Can not get http response stream. {0}", LogRequetId(req.RequestId));
                //LogHelper.OnlineLogger.Error(ex);

                Debug.WriteLine("Can not get http response stream. {0}", LogRequetId(req.RequestId));
                Debug.WriteLine(ex);
        ...
        private void ReceiveResponseStreamInBlocks(Stream source, HttpResponse resp, int receiveBlockSize)
        {
            ...
            //LogHelper.OnlineLogger.DebugFormat("Start to receive response stream, id={0}. {1}", sourceHash, LogRequetId(resp.ResponseId));
            Debug.WriteLine("Start to receive response stream, id={0}. {1}", sourceHash, LogRequetId(resp.ResponseId));
            ...
            while (true)
            {
                ...
                {
                    //LogHelper.OnlineLogger.ErrorFormat("Receive stream is failed in the middle, id={0}, ex={1}. {2}", sourceHash, ex.ToString(), LogRequetId(resp.ResponseId));
                    Debug.WriteLine("Receive stream is failed in the middle, id={0}, ex={1}. {2}", sourceHash, ex.ToString(), LogRequetId(resp.ResponseId));
                    /*if (ex.Status == WebExceptionStatus.Timeout)
                    {
                ...
                Debug.WriteLine(string.Format("Data {0}/{1}, id={2}. {3}", se.Offset, resp.OriginalResponse.ContentLength, sourceHash, LogRequetId(resp.ResponseId)));
            }
        ...
        {
            //LogHelper.OnlineLogger.ErrorFormat("Handle web exception {0}. {1}", ex.Status, LogRequetId(request.RequestId));
            //LogHelper.OnlineLogger.Error(ex);

            Debug.WriteLine("Handle web exception {0}. {1}", ex.Status, LogRequetId(request.RequestId));
            Debug.WriteLine(ex);
        ...
        private const string httpLibraryName = "HttpLibrary.Net";
        private string userAgent = null;
        private void SetUserAgent(HttpWebRequest httpRequest)
        {
            if (userAgent == null)
            {
                AssemblyName name = new AssemblyName(httpLibraryName);
                var thisAssembly = Assembly.Load(name);

                var version = thisAssembly.GetName().Version.ToString();

                var devInfo = httpLibPlatform.DeviceInfo;
                userAgent = string.Format("{0} ({1};{2}) {3}/{4}", devInfo.Application, devInfo.Device, devInfo.System, httpLibraryName, version);
